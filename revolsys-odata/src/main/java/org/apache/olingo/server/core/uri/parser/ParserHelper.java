...
public class ParserHelper {
  ...
  static {
    /*
     * Enum and null are not present in the map. These have to be handled
     * differently.
     */
    final Map<TokenKind, EdmPrimitiveTypeKind> temp = new EnumMap<>(TokenKind.class);
    temp.put(TokenKind.BooleanValue, EdmPrimitiveTypeKind.Boolean);
    temp.put(TokenKind.StringValue, EdmPrimitiveTypeKind.String);
    // Very large integer values are of type Edm.Decimal but this is handled
    // elsewhere.
  ...
  protected static boolean bws(final UriTokenizer tokenizer) {
    return tokenizer.nextWhitespace();
  }

  private static List<UriParameter> compoundKey(final UriTokenizer tokenizer,
    final EdmEntityType edmEntityType, final Edm edm, final EdmType referringType,
    final Map<String, AliasQueryOption> aliases) throws UriParserException, UriValidationException {

    final List<UriParameter> parameters = new ArrayList<>();
    ...
    do {
      ...
      parameters
        .add(keyValuePair(tokenizer, keyPredicateName, edmEntityType, edm, referringType, aliases));
  ...
  private static UriParameter createUriParameter(final EdmProperty edmProperty,
    final String parameterName, final String literalValue, final Edm edm,
    final EdmType referringType, final Map<String, AliasQueryOption> aliases)
  ...
  private static AliasQueryOption getKeyAlias(final String name, final EdmProperty edmProperty,
    ...
    if (aliases.containsKey(name)) {
      return parseAliasValue(name, edmProperty.getType(), edmProperty.isNullable(),
        edmProperty.isCollection(), edm, referringType, aliases);
    } else {
  ...
  private static UriParameter keyValuePair(final UriTokenizer tokenizer,
    final String keyPredicateName, final EdmEntityType edmEntityType, final Edm edm,
    final EdmType referringType, final Map<String, AliasQueryOption> aliases)
    throws UriParserException, UriValidationException {
  ...
  protected static TokenKind next(final UriTokenizer tokenizer, final TokenKind... kinds) {
  ...
  private static boolean nextPrimitiveTypeValue(final UriTokenizer tokenizer,
    ...
    if (tokenizer.next(TokenKind.ParameterAliasName)) {
      ...
      // A decimal value should not be parsed as integer and let the tokenizer
      // stop at the decimal point.
      return tokenizer.next(TokenKind.DoubleValue) || tokenizer.next(TokenKind.DecimalValue)
  ...
  protected static TokenKind nextPrimitiveValue(final UriTokenizer tokenizer) {
    return next(tokenizer, TokenKind.NULL, TokenKind.BooleanValue, TokenKind.StringValue,

      // The order of the next seven expressions is important in order to avoid
      // finding partly parsed tokens (counter-intuitive as it may be, even a
      // GUID may start with digits ...).
      ...
      TokenKind.DurationValue, TokenKind.BinaryValue, TokenKind.EnumValue,

      // Geography and geometry literals are defined to be primitive,
      // although they contain several parts with their own meaning.
      TokenKind.GeographyPoint, TokenKind.GeometryPoint, TokenKind.GeographyLineString,
      TokenKind.GeometryLineString, TokenKind.GeographyPolygon, TokenKind.GeometryPolygon,
      TokenKind.GeographyMultiPoint, TokenKind.GeometryMultiPoint,
      TokenKind.GeographyMultiLineString, TokenKind.GeometryMultiLineString,
      TokenKind.GeographyMultiPolygon, TokenKind.GeometryMultiPolygon,
      TokenKind.GeographyCollection, TokenKind.GeometryCollection//
    );
  }

  protected static AliasQueryOption parseAliasValue(final String name, final EdmType type,
    final boolean isNullable, final boolean isCollection, final Edm edm,
    final EdmType referringType, final Map<String, AliasQueryOption> aliases)
    throws UriParserException, UriValidationException {
    ...
    if (alias != null && alias.getText() != null) {
      ...
      if (kind == null
        ...
        // The alias value is not an allowed literal value, so parse it again as
        // expression.
  ...
  protected static List<UriParameter> parseFunctionParameters(final UriTokenizer tokenizer,
  ...
  protected static List<UriParameter> parseKeyPredicate(final UriTokenizer tokenizer,
    final EdmEntityType edmEntityType, final EdmNavigationProperty partner, final Edm edm,
    final EdmType referringType, final Map<String, AliasQueryOption> aliases)
    throws UriParserException, UriValidationException {
    ...
    if (partner != null) {
      // Prepare list of potentially missing keys to be filled from referential
      // constraints.
    ...
    if (keyPropertyRefs.size() - referencedNames.size() == 1) {
      for (final EdmKeyPropertyRef candidate : keyPropertyRefs) {
        if (referencedNames.get(candidate.getName()) == null) {
          final UriParameter simpleKey = simpleKey(tokenizer, candidate, edm, referringType,
            aliases);
          if (simpleKey != null) {
            keys.add(simpleKey);
          }
  ...
  protected static List<UriParameter> parseNavigationKeyPredicate(final UriTokenizer tokenizer,
    final EdmNavigationProperty navigationProperty, final Edm edm, final EdmType referringType,
    final Map<String, AliasQueryOption> aliases) throws UriParserException, UriValidationException {
  ...
  protected static int parseNonNegativeInteger(final String optionName, final String optionValue,
    ...
    try {
      ...
      throw new UriParserSyntaxException("Illegal value of '" + optionName + "' option!", e,
        UriParserSyntaxException.MessageKeys.WRONG_VALUE_FOR_SYSTEM_QUERY_OPTION, optionName,
        optionValue);
    }
  ...
  protected static void requireTokenEnd(final UriTokenizer tokenizer) throws UriParserException {
    requireNext(tokenizer, TokenKind.EOF);
  }

  private static UriParameter simpleKey(final UriTokenizer tokenizer,
    final EdmKeyPropertyRef edmKeyPropertyRef, final Edm edm, final EdmType referringType,
    final Map<String, AliasQueryOption> aliases) throws UriParserException, UriValidationException {
    ...
    if (nextPrimitiveTypeValue(tokenizer,
      ...
      final String literalValue = tokenizer.getText();
      ParserHelper.requireNext(tokenizer, TokenKind.CLOSE);
      return createUriParameter(edmProperty, edmKeyPropertyRef.getName(), literalValue, edm,
        referringType, aliases);
    } else {
      return null;
    }
  ...
  protected static void validateFunctionParameterFacets(final EdmFunction function,
    final List<UriParameter> parameters, final Edm edm, final Map<String, AliasQueryOption> aliases)
    throws UriParserException, UriValidationException {
    for (final UriParameter parameter : parameters) {
      ...
      if ((kind == EdmTypeKind.PRIMITIVE || kind == EdmTypeKind.DEFINITION
        || kind == EdmTypeKind.ENUM) && !edmParameter.isCollection()) {
  ...
  protected static void validateFunctionParameters(final EdmFunction function,
    final List<UriParameter> parameters, final Edm edm, final EdmType referringType,
    final Map<String, AliasQueryOption> aliases) throws UriParserException, UriValidationException {
    for (final UriParameter parameter : parameters) {
