...
public class MethodImpl implements Method {
  ...
  public <T> T accept(final ExpressionVisitor<T> visitor)
    ...
    final List<T> userParameters = new ArrayList<>();
    if (this.parameters != null) {
      for (final Expression parameter : this.parameters) {
        userParameters.add(parameter.accept(visitor));
      }
    }
    return visitor.visitMethodCall(this.method, userParameters);
  }
  ...
  public List<Expression> getParameters() {
    return this.parameters == null ? Collections.<Expression> emptyList()
